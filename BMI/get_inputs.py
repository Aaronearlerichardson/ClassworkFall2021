...
def get_weight_input():
    ...
    raw_in = input("Type a weight in pounds or kilograms (default is "
                   "kilograms)\nexample: 150.2 lbs \n "
                   "Type your input-->")
...
def get_height_input():
    ...
    height_patterns = ("meters", "[Mm](?:eters?)?", "(?:in)(?:ch(?:es)?)?")
    # no one types IN for inches
    raw_in = input("Type a height in inches or meters (default is "
...
def match_words(raw_in, patterns):
    """a pattern matching function to return a dictionary with a measurement
    and units given a string with both values and patterns matching units """
    ...
    if match:
        ...
        for pattern in patterns:
            if re.match(pattern + " *$", unit):
                ...
                typo_list = [typo for typo in possible_typos
                             if re.match(pattern + " *$", typo)]
                if typo_list:
                    raw_in = input("you typed {}, did you mean {}?\nTry "
                                   "again-->".format(unit, typo_list[0]))
                    return match_words(raw_in, patterns)
    raise NameError("invalid input given: {}".format(raw_in))


def edits_one(word):
    """function that returns the set of all possible edits that are one letter
    edit away from word input """
